/**
 * Browser copy of backend bookingAIEngine — used when /api/ai and /public/calculate-booking are unreachable.
 * Keep in sync with backend-repo/server/services/bookingAIEngine.js (logic only).
 */
(function () {
  function hourIST(d) {
  ...
  function countEligibleWorkersStub() {
    return 4;
  }

  function calculateBookingEngineLocal(input) {
    const {
      service = "",
      category = "cleaning",
      problem = "",
    ...
    const city = (customer && customer.city) || "Hyderabad";
    const membership = String((customer && customer.membership) || "free").toLowerCase();
    const isPlus = membership === "plus" || membership === "premium";

    const prob = analyzeProblem(problem);
    const workersNear = countEligibleWorkersStub();
    ...
    if (prob.severity === "High") recommendedPlan = "Fast Delivery";
    else if (prob.severity === "Low" && cartValue < 800) recommendedPlan = "Budget Flexible";
    ...
    return {
      ...
      upgradeProposal,
      meta: { service, category, city, cartValue: Number(cartValue) || 0, source: "local-fallback" }
    };
  }

  window.calculateBookingEngineLocal = calculateBookingEngineLocal;
})();
